...
distrobox enter "$VALERIO_CONTAINER_NAME" -- bash -c "export WINEPREFIX=\"$VALERIO_PREFIX_DIR\"; export WINEDLLOVERRIDES=\"winemenubuilder.exe=d\"; export PATH=\"$WINE_CUSTOM_BIN:\$PATH\"; cd \"$VALERIO_PREFIX_DIR/drive_c/Program Files/Steinberg/Dorico6\"; if [ -n \"\$1\" ]; then wine 'Dorico6.exe' \"\$1\"; else wine 'Dorico6.exe'; fi" _ "$WIN_PATH"
